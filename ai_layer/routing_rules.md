...
| 投放表现(V1.0.1) | 投放/投放表现/投放消耗/店铺被投/店铺绑定/投放贡献/投放费比/综合费比/投放效率/全店效率/ROI | get_advertising_summary | scope_key, dates |
| 投放环比(V1.0.1) | 投放+环比/比前面/比之前 | compare_advertising | scope_key, dates |
| 日报/周报(V1.0 模板) | 日报/周报/经营报表/三板块/抖音弹动官方旗舰店/抖音弹动个人护理旗舰店/经营类型/自营直播/自营商品/达人直播/达人短视频/橱窗 | get_business_report | start_date, end_date |
| 异常(V1.1 S2) | 异常/哪里异常/多严重/是否持续/影响多大 | get_anomalies / get_entity_anomalies / get_anomaly_summary | domain, entity, dates |
| 诊断定位(V1.1 S3) | 为什么下降/拖累/漏斗/曝光点击成交/投放诊断 | get_diagnostic_result / get_change_decomposition / get_funnel_diagnosis / get_advertising_diagnosis | domain, entity, dates |
| 机会(V1.1 S4) | 机会/增长机会/机会评分 | get_growth_opportunities / get_entity_opportunity / get_opportunity_summary | domain, dates |
| 优先级/行动(V1.1 S5) | 今日风险/机会TOP/行动清单/每日简报/为什么排第一 | get_daily_risk_priorities / get_daily_opportunity_priorities / get_daily_action_list / get_daily_business_brief | dates |
| 主数据(V1.3 S3) | 属于哪个公司商品/同款/品线有哪些/未归属/冲突 | resolve_master_product / get_master_product_members / get_product_line_members / get_unmapped_products / get_mapping_conflicts | entity |

## 2. 意图优先级（歧义时）

1. 明确"日报/周报/经营报表/三板块/经营类型" → get_business_report（V1.0 模板专用，跨店汇总）
2. 明确"投放/ROI/店铺被投/店铺绑定/投放费比/投放效率" 且非排名 → get_advertising_summary / compare_advertising（V1.0.1 专用）
2. 明确"环比/比之前/增长多少/下降多少" → compare_business / compare_advertising（投放相关用后者）
3. 明确"TOP/排名/增长最快/下降最大/排名变化" → rank_*
4. 明确"贡献/占比" → *_contribution
5. 明确"表现/成交/汇总/多少" → *_summary / get_business_summary
6. 仅"数据/覆盖/导入/指标" → 基础目录工具
...
| 日报（V1.0 模板）| start_date=end_date=用户指定日期或 coverage.max_date |
| 周报（V1.0 模板）| 默认 7 天；用户说"上周/本周"按周一~周日；其余给起止日；与 get_business_report 配对 |

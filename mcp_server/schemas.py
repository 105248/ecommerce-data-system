...
def check_shop(shop_name, required=False):
    """店铺校验（V1.3 多店）：
    - shop_name 非空 → 必须在 meta.shop 中存在，否则 UNKNOWN_SHOP。
    - shop_name 为空且 required=True 且多店启用 → SHOP_REQUIRED（禁止把单店伪装成平台汇总）。
    - 单店环境 required=True 且未指定 → 自动使用唯一店铺语义由调用方决定（此处仅校验）。"""
    import database as _db
    if shop_name is not None:
        rows = _db.query("SELECT 1 FROM meta.shop WHERE shop_name = %s AND enabled", (shop_name,))
        if not rows:
            raise ArgError("UNKNOWN_SHOP",
                           "未知店铺 '{}'。可用店铺：{}".format(shop_name, "、".join(
                               [r["shop_name"] for r in _db.query("SELECT shop_name FROM meta.shop WHERE enabled ORDER BY shop_name")])))
        return
    if required:
        shops = _db.query("SELECT shop_name FROM meta.shop WHERE enabled ORDER BY shop_name")
        if len(shops) > 1:
            raise ArgError("SHOP_REQUIRED",
                           "多店环境必须指定店铺。可用：{}（本阶段不做两店汇总）".format("、".join([r["shop_name"] for r in shops])))

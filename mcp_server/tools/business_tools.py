# -*- coding: utf-8 -*-
"""经营总览 Tools：get_business_summary / compare_business / get_business_report"""
import database
import schemas
import config
...
def get_business_report(start_date=None, end_date=None):
    """日报/周报数据模板 V1.0：三板块(抖音整体/抖音弹动官方旗舰店/抖音弹动个人护理旗舰店)×六行
    (整体/自营直播/自营商品/达人直播/达人短视频/橱窗)。
    指标：成交金额/成交退款金额/结算金额/退款率/投放消耗/投放费比(剔除退款、店铺绑定)。
    比率已×100 返回百分比数值。周期任意，日报传单日，周报传一周区间。无利润计算。"""
    schemas.validate_period(start_date, end_date)

    rows = database.query(
        "SELECT section, business_type, user_pay_amount, refund_amount, settlement_amount, "
        "refund_rate, ad_spend, ad_fee_rate "
        "FROM mart.get_business_report(%s::date, %s::date)",
        (start_date, end_date),
    )
    if not rows:
        return schemas.err("get_business_report", "NO_DATA", "该区间无数据")
    ...
    for r in rows:
        data.append({
            ...
            "成交金额": r.get("user_pay_amount"),
            "成交退款金额": r.get("refund_amount"),
            "结算金额": r.get("settlement_amount"),
            "退款率_pct": _fmt_ratio(r.get("refund_rate")),
            "投放消耗": r.get("ad_spend"),
            "投放费比_pct": _fmt_ratio(r.get("ad_fee_rate")),
        })
    return schemas.ok("get_business_report", data)

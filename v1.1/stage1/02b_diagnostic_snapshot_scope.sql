-- ============================================================================
-- V1.1 阶段1｜经营指标诊断基础层
-- 02b_diagnostic_snapshot_scope.sql（_diag_scope 经营 Scope 域）
-- ============================================================================
-- 对象 = 18 个 Scope（p_scope_key 指定单个或 NULL=全部）
-- 贡献 = 占全店比重（白名单贡献指标），denominator_type = store
-- ============================================================================
DROP FUNCTION IF EXISTS mart._diag_scope(text,date,date,date,date,text);

CREATE FUNCTION mart._diag_scope(
...
COMMENT ON FUNCTION mart._diag_scope(text,date,date,date,date,text) IS 'V1.1 经营Scope域诊断快照（内部函数，不对外授权）。';
